...
<http://the/link/#thelink> a schema:Article;
  ...
  context "with many categories" do
    resource = Resource.new("http://the/link/#theLink", "Article")
      .add_property("headline", "About the project")
      .add_property("articleSection", ["Projects", "Open Data", "Open contracts"])
    turtle = Turtle.new(prefixes).add_resource(resource)

    it "should be equal to" do
      expect(turtle.to_s).to be == "@prefix rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#> .\n@prefix schema: <http://schema.org/> .\n\n<http://the/link/#theLink> a schema:Article;\n   schema:articleSection \"Projects\",\n     \"Open Data\",\n     \"Open contracts\";\n   schema:headline \"About the project\" .\n"
    end
  end
